...
class HaltingQwen3VLModelOutputWithPast(ModelOutput):
    r"""
...
class HaltingModule(nn.Module):
    def __init__(self, config):
        ...
        self.halting_query_tokens = nn.Parameter(torch.randn(self.halting_query_num, config.hidden_size))
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def __init__(self, config: Qwen3VLTextConfig):
        ...
        self.as_student = getattr(config, 'as_student', False)
        if self.as_student:
            config.num_hidden_layers = 4 + config.num_preserved_layers
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #----------------- for teacher forcing start -----------------
        assert self.num_preserved_layers * self.num_loop == 24, \
            f'num_preserved_layers * num_loop need be 24, but now {self.num_preserved_layers=} {self.num_loop=}'
        ...
        return HaltingModelOutputWithPast(
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> Union[tuple, HaltingQwen3VLModelOutputWithPast]:
        r"""
        image_grid_thw (`torch.LongTensor` of shape `(num_images, 3)`, *optional*):
            The temporal, height and width of feature shape of each image in LLM.
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
    ...
    def cache_inputs(
        ...
        **kwargs,
        ...
        #  helper：Tensor → clone+detach
        def _clone_detach(x):
            return x.clone().detach() if isinstance(x, torch.Tensor) else x

        cache["input_ids"] = input_ids
        cache["position_ids"] = _clone_detach(position_ids)
        cache["attention_mask"] = _clone_detach(attention_mask)

        cache["past_key_values"] = _clone_detach(past_key_values)
        cache["inputs_embeds"] = _clone_detach(inputs_embeds)
        cache["cache_position"] = _clone_detach(cache_position)
        cache["visual_pos_masks"] = _clone_detach(visual_pos_masks)

        if deepstack_visual_embeds is None:
            cache["deepstack_visual_embeds"] = None
        else:
            cache["deepstack_visual_embeds"] = [
                x.clone().detach() for x in deepstack_visual_embeds
            ]

        for k, v in kwargs.items():
            if isinstance(v, torch.Tensor):
                cache[k] = v.clone().detach()
            else:
                cache[k] = v

        self._lm_input_cache = cache
        return cache

    def pop_inputs(self):
        if not getattr(self, "_lm_input_cache", None):
            return None
